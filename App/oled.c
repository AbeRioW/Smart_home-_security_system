...
void OLED_ShowTempHumidity(DHT11_Data_TypeDef *data)
{
	...
	if (data->humidity_int >= 100)
	{
		OLED_ShowNum(54, 0, data->humidity_int, 3, 8, 1);
	}
	else if (data->humidity_int >= 10)
	{
		OLED_ShowNum(54, 0, data->humidity_int, 2, 8, 1);
	}
	else
	{
		OLED_ShowNum(54, 0, data->humidity_int, 1, 8, 1);
	}
	
	OLED_ShowChar(66, 0, '.', 8, 1);
	OLED_ShowNum(72, 0, data->humidity_dec, 1, 8, 1);
...
{
	uint16_t co2_int, co2_dec;
	uint8_t i;
	
	co2_int = (uint16_t)co2;
	co2_dec = (uint16_t)((co2 - co2_int) * 10);
	...
	OLED_ShowString(0, 8, (uint8_t *)"CO2:", 8, 1);
	
	if (co2_int >= 100)
	{
		OLED_ShowNum(24, 8, co2_int, 3, 8, 1);
		OLED_ShowChar(48, 8, '.', 8, 1);
		OLED_ShowNum(54, 8, co2_dec, 1, 8, 1);
		OLED_ShowString(60, 8, (uint8_t *)"mg/m3", 8, 1);
	}
	else if (co2_int >= 10)
	{
		OLED_ShowNum(24, 8, co2_int, 2, 8, 1);
		OLED_ShowChar(42, 8, '.', 8, 1);
		OLED_ShowNum(48, 8, co2_dec, 1, 8, 1);
		OLED_ShowString(54, 8, (uint8_t *)"mg/m3", 8, 1);
	}
	else
	{
		OLED_ShowNum(24, 8, co2_int, 1, 8, 1);
		OLED_ShowChar(30, 8, '.', 8, 1);
		OLED_ShowNum(36, 8, co2_dec, 1, 8, 1);
		OLED_ShowString(42, 8, (uint8_t *)"mg/m3", 8, 1);
	}
...
void OLED_ShowMQ5Value(uint16_t value)
{
	...
	OLED_ShowNum(24, 16, value, 4, 8, 1);
	OLED_ShowString(52, 16, (uint8_t *)"/4095", 8, 1);
